...
```bash
uv run --group audit fis-catalogs audit match --project project.toml
```
...
```bash
uv run --group audit fis-catalogs audit report --project project.toml --force
```
...
The audit commands keep automatic cleanup conservative. They auto-match only
